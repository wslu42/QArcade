...
game_title="signal rescue"

levels={
  {
    name="1 emergency ping",
    target={ ["001"]=16 },
    pass=16,
    hint="power beacon 001",
    lesson="single beacon online"
  },
  {
    name="2 signal split",
    target={ ["000"]=8,["001"]=8 },
  ...
  },
  {
    ...
    pass=11,
    hint="sync 000 + 011",
    lesson="relay bits linked"
  },
  {
    name="4 deep-space link",
    target={ ["000"]=8,["111"]=8 },
    pass=11,
    hint="sync 000 + 111",
    lesson="rescue channel stable"
  }
}
--------- quantum router game ---------
...
function beacon_center(i)
  local response=layout.response
  local col=(i-1)%4
  local row=flr((i-1)/4)
  return 16+col*32,response.y+20+row*27
end

function draw_target_links()
  local last=-1
  for i=1,#states do
    local state=states[i]
    if (level.target[state] or 0)>0 then
      if last>0 then
        local x1,y1=beacon_center(last)
        local x2,y2=beacon_center(i)
        line(x1,y1,x2,y2,5)
      end
      last=i
    end
...
function draw_beacon(i,state,wanted,count)
  local cx,cy=beacon_center(i)
  local edge=wanted and 10 or 5
  local glow=count>0 and 11 or 1
  local power=min(5,flr((count+1)/3))

  if wanted then circ(cx,cy,10,edge) end
  line(cx,cy-7,cx,cy-4,edge)
  pset(cx-1,cy-8,edge)
  pset(cx+1,cy-8,edge)

  rectfill(cx-7,cy-4,cx+7,cy+6,1)
  rect(cx-7,cy-4,cx+7,cy+6,edge)
  rectfill(cx-5,cy-2,cx-5+power*2,cy+1,glow)
  line(cx-4,cy+7,cx+4,cy+7,5)

  print(state,cx-6,cy-3,7)
  if count>0 then
    print(count,cx-3,cy+3,11)
  else
    print("-",cx-1,cy+3,5)
  end
end

function draw_beacon_network()
  local response=layout.response
  print("rescue beacon net",3,response.y+2,10)
  print("* target # signal",59,response.y+2,6)
  draw_target_links()
...
function draw_complete()
  cls(1)
  print_centered("signal rescue",42,11)
  print_centered("all beacons online",58,7)
  print_centered("crew channel restored",67,7)
