...
levels={
  {
    ...
    target={ ["001"]=16 },
    pass=16,
    hint="crack vault 001",
    lesson="clean getaway"
  },
  {
    name="2 twin take",
    target={ ["000"]=8,["001"]=8 },
...
function draw_vault(x,y,state,wanted,count)
  local edge=wanted and 10 or 5
  local door=count>0 and 12 or 6

  -- chunky 26x20 vault tile
  rectfill(x,y,x+25,y+19,1)
  rect(x,y,x+25,y+19,edge)
  print(state,x+7,y+1,6)

  -- 8x8 steel door with wheel and hinges
  rectfill(x+3,y+8,x+11,y+16,door)
  rect(x+3,y+8,x+11,y+16,7)
  circ(x+7,y+12,2,1)
  pset(x+7,y+12,7)
  pset(x+4,y+10,7)
  pset(x+4,y+14,7)
  ...
  -- compact shot count; cyan means visited
  if count>0 then
    print(count,x+16,y+13,11)
  else
    print("-",x+19,y+13,5)
  end
end

function draw_heist_board()
  local response=layout.response
  print("vault network",3,response.y+2,10)
  print("* loot  # visits",62,response.y+2,6)
  ...
  for i=1,#states do
    ...
    local row=flr((i-1)/4)
    local x=3+col*31
    local y=response.y+12+row*25
    draw_vault(x,y,state,(level.target[state] or 0)>0,counts[state] or 0)
  end
end

function draw_complete()
  cls(1)
  print_centered("heist complete",42,11)
  print_centered("all vaults cracked",58,7)
  print_centered("quantum clean getaway",67,7)
